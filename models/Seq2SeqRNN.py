...
class EncoderRNN(nn.Module):
    def __init__(self, embedder, num_layers=1, bidirectional=False, device='cpu'):
        ...
        self.gru = nn.GRU(self.hidden_size, self.hidden_size, num_layers=num_layers, bidirectional=bidirectional).to(self.device)
...
class AttnAutoEncoderRNN(nn.Module):
    def __init__(self, embedder, max_length, dropout_p=0.1, device='cpu', teacher_forcing_ratio = 0.5):
        super(AttnAutoEncoderRNN, self).__init__()
        self.max_length = max_length  # longueur maximum de la prediction
        self.device = device
        print(self.device)
        self.embedder = embedder
        self.encoder = EncoderRNN(embedder, device=self.device)
        self.decoder = AttnDecoderRNN(embedder, self.max_length, device=self.device)
    ...
    def forward(self, source):
        ...
        # print('last one')
        encoder_output, encoder_hidden = self.encoder(
            torch.stack([self.eos_token.to(torch.int64)]*batch_size, dim=0).unsqueeze(0).to(self.device), encoder_hidden)
